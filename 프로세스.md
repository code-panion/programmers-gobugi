...
## 🔍 문제에 접근한 방식과 이유
- 최댓값 확인 후 해당 값이 아니면 큐에서 계속 돌린다 (pop(0) 및 append)
- 최댓값일 경우 이전 위치 인덱스를 저장하여 특정 프로세스의 위치 확인

## ❌ 오답인 경우 (해당하는 경우에만 작성)
- priorities, lst 과정에서 뭔가 엉킴
- 첫 mas 뒷 개수로 접근 후 앞 뒤 숫자기준 비교도 시도해보았으나 실패 (구현 복잡성)
